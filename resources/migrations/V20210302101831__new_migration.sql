ALTER TABLE institutions_scorers
    RENAME TO __institutions_scorers;
ALTER TABLE institutions_taf_administrators
    RENAME TO __institutions_taf_administrators;
...
(
    ...
    currency_code smallint             DEFAULT NULL,
...
INSERT INTO countries (id, iso, short_name, full_name, iso3, currency_code, phone_code)
...
(
    id           uuid constraint scorers_pk primary key,
    user_id      uuid    not null  constraint scorers_user_fk references accounts,
    country_code char(2) not null constraint scorers_c_fk references countries (iso)
);

CREATE UNIQUE INDEX scorers_user ON scorers (user_id, country_code);

CREATE TABLE taf_administrators
(
    id           uuid constraint taf_administrators_pk primary key,
    user_id      uuid    not null constraint taf_administrators_user_fk references accounts,
    country_code char(2) not null  constraint taf_administrators_c_fk references countries (iso)
);

CREATE UNIQUE INDEX taf_administrators_user ON taf_administrators (user_id, country_code);

ALTER TABLE institutions DROP COLUMN address;
ALTER TABLE institutions DROP COLUMN city;
ALTER TABLE institutions DROP COLUMN country;
ALTER TABLE institutions DROP COLUMN zip_code;

ALTER TABLE institutions ADD country_code char(2);
ALTER TABLE institutions ADD CONSTRAINT institutions_c_fk FOREIGN KEY (country_code) REFERENCES countries (iso);

ALTER TABLE requests ADD country_code char(2);
ALTER TABLE requests ADD CONSTRAINT requests_c_fk FOREIGN KEY (country_code) REFERENCES countries (iso);

ALTER TABLE accounts ADD country_code char(2);
ALTER TABLE accounts ADD CONSTRAINT accounts_c_fk FOREIGN KEY (country_code) REFERENCES countries (iso);